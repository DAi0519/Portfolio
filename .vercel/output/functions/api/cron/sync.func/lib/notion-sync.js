...
const preserveNotionInlineStyles = (text, annotations) => {
    if (text.match(/^\s*$/))
        return text;
...
const preserveToggleBlocks = (blocks) => {
...
function getSyncContext() {
    ...
    n2m.annotatePlainText = preserveNotionInlineStyles;
    return {
        notionApiKey,
        notionDbId,
        n2m,
...
export async function runNotionSync() {
    ...
    for (const page of allPages.results) {
        ...
        processedContent = processedContent.replace(/\[([^\]]+\.(mp4|mov|webm))\]\(([^)]+)\)/gi, (_, _filename, _ext, url) => `![VIDEO](${url})`);
        const finalPayload = {
            id: pageId,
            title,
            date,
